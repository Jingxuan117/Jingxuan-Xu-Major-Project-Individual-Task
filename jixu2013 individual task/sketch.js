// A slightly shake animation effect is achieved in the artwork by adding the drawing of a Perlin noise based animated grid. 
// The x and y positions of each grid point in the grid were calculated and  using Perlin noise to produce a dynamic effect. 
// This shaking effect gives a slight random shake to the grid lines, 
// making the animation look more realistic and realizing the effect of dynamic Lego blocks, 
// thus creating a dynamic visual effect using Perlin noise and random value.
...
// draw a grid with perlin noise animation
function drawGrid() {
    push();
    background(254, 251, 230);
    // distribute x positions across the canvas width
    for (let i = 0; i < cols; i++) {
        xPositions[i] = map(i, 0, cols - 1, gridSize, width - gridSize);
    }
    // distribute y positions across the canvas height
    for (let j = 0; j < rows; j++) {
        yPositions[j] = map(j, 0, rows - 1, gridSize, height - gridSize);
    }

    // apply noise to the x & y positions to create a animation shaking effect
   for (let i = 1; i < cols - 1; i++) {
        let noiseValue = noise(i * noiseScale, time);
        let adjustment = map(noiseValue, 0, 1, -gridSize * 1.2, gridSize * 1.2);
        xPositions[i] += adjustment;
    }
    for (let j = 1; j < rows - 1; j++) {
        let noiseValue = noise(j * noiseScale, time);
        let adjustment = map(noiseValue, 0, 1, -gridSize * 1.2, gridSize * 1.2);
        yPositions[j] += adjustment;
    }

    // draw lines inside the cells grid if the noise value exceeds a threshold values
    for (let i = 0; i < cols - 1; i++) {
        for (let j = 0; j < rows - 1; j++) {
            let noiseValue = noise((i + 0.5) * noiseScale, (j + 0.5) * noiseScale, time);
            if (noiseValue > noiseThreshold) {
                let x1 = xPositions[i];
                let x2 = xPositions[i + 1];
                let y1 = yPositions[j];
                let y2 = yPositions[j + 1];
                let cellWidth = x2 - x1; // cell width
                let cellHeight = y2 - y1; // cell height
                let spacing = min(cellWidth, cellHeight) * 0.3; // spacing for lines
                // stroke color and thickness
                stroke(33, 93, 221);
                strokeWeight(1.5);
                let numLines = 2; // number of vertical lines
                let lineSpacing = cellWidth / (numLines + 1);
                //vertical lines in the cells grid 
                for (let k = 1; k <= numLines; k++) {
                    let lineX = x1 + k * lineSpacing;
                    line(lineX, y1, lineX, y2);
                }
            }
        }
    }
    // No fill for the grid lines
    noFill();
    // vertical lines
    for (let i = 0; i < cols; i++) {
        line(xPositions[i], gridSize, xPositions[i], height - gridSize);
    }
    // horizontal lines
    for (let j = 0; j < rows; j++) {
        line(gridSize, yPositions[j], width - gridSize, yPositions[j]);
    }
